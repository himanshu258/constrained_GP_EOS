...
def Kernel(x1, x2, l1, l2, sigma_f, sigma_n1, sigma_n2, constraints=1):
    ...
    K = np.block([[K11,constraints*K12],[constraints*K21,K22]])
...
def mle(theta, x_train, y_train, mean_regression, constraints=1):
    ...
    n = x_train.shape[0]      
    K = Kernel(x_train, x_train, 10**l1, 10**l2, 10**sigma_f, 10**sigma_n1, 10**sigma_n2, constraints)   + np.eye(2*n)*1e-10
...
def predict(theta, x_train, y_train, x_test, n_test, degree, constraints=1):
    ...
    k12 = Kernel(x_train, x_test, l1, l2, sigma_f, sigma_n1, sigma_n2, constraints)
    k21 = Kernel(x_test, x_train, l1, l2, sigma_f, sigma_n1, sigma_n2, constraints)
    k22 = Kernel(x_test, x_test, l1, l2, sigma_f, sigma_n1, sigma_n2, constraints)
    K = Kernel(x_train, x_train, l1, l2, sigma_f, sigma_n1, sigma_n2, constraints) + np.eye(2 * n) * 1e-10
    L = np.linalg.cholesky(K)
...
def predict_prime(theta, x_train, y_train, x_test, n_test, degree, constraints=1):
        ...
        k12 = Kernel_prime(x_train, x_test, 10**l1, 10**l2, 10**sigma_f, 10**sigma_n1, 10**sigma_n2, constraints)  
        k21 = Kernel_prime(x_test, x_train, 10**l1, 10**l2, 10**sigma_f, 10**sigma_n1, 10**sigma_n2, constraints)
        k22 = Kernel_prime(x_test, x_test, 10**l1, 10**l2, 10**sigma_f, 10**sigma_n1, 10**sigma_n2, constraints) 
        K = Kernel_prime(x_train, x_train, 10**l1, 10**l2, 10**sigma_f, 10**sigma_n1, 10**sigma_n2, constraints) +  np.eye(2*n)*1e-10
        L = np.linalg.cholesky(K)
